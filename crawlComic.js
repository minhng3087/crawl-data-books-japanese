...
const scraperObject = {
  url: "https://www.fahasa.com/foreigncategory/other-languages/sach-tieng-nhat/culture-travel.html?order=num_orders&limit=48&p=1",
    ...
    async function scrapeCurrentPage() {
      ...
      let thumbs = await page.$$eval(
        ...
        (links) => {
          ...
          links = links.map((el) => el.querySelector('.product.images-container a img').src);
          return links;
        }
      );
      ...
      let urls = await page.$$eval(
        ...
        (links) => {
          // Extract the links from the data
          links = links.map((el) => el.querySelector(".product.images-container a").href);
          return links;
        }
      );
      let pagePromise = (link) =>
        new Promise(async (resolve, reject) => {
          ...
          if ((await newPage.$('.product-essential-detail h1')) !== null) {
